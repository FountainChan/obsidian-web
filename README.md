...
Node 18+ or Bun — depends on the package, not one or the other for the whole repo:

- `src/client-mobile` tests, the `runtime-server` backend, and the Cloudflare deployment's
  *build* script (`build-assets.sh`) all invoke `node`.
- `src/sync-server` runs entirely on `bun` (`bun index.js` / `bun test`).
- The Cloudflare deployment's own **test suite** also requires `bun` — its `npm test` runs
  `bun test`, even though its *build* is `bash` + `node` (see "Key files" below). Don't assume
  `bun` works for every package just because one package needs it.
...
| `src/deployments/cloudflare/test/` | `bun test` — proxy, build-assets, build-system-plugins, and install-plugin tests |
...
over Obsidian's code. **This repository** does not contain and does not distribute Obsidian's
source — the setup scripts download it and apply a small set of documented patches to **your
local copy** at install time. The **public live demo**, however, *does* serve Obsidian's bundle
(so it can run in visitors' browsers) — see `build-assets.sh`, which copies it into the deployed
static assets. If the Obsidian team has concerns about that, see the Disclaimer above.
